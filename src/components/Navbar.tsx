  ...
  return (
    <header className="fixed top-4 left-4 right-4 z-50 transition-all duration-300 max-w-5xl mx-auto glass rounded-2xl px-6 py-3 flex justify-between items-center">
      ...
      <div className="flex items-center gap-4">
        ...
        <a
          href="https://drive.google.com/uc?export=download&id=1Z4OwnpUtVhITRHefXbh8dtag4v9eyTqW"
          target="_blank"
          rel="noopener noreferrer"
          className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs px-4 py-2 rounded-full font-bold transition-colors shadow-lg shadow-indigo-900/20 hidden sm:block"
        >
          DOWNLOAD CV
        </a>
      ...
      {isOpen && (
        <div className="absolute top-16 left-0 right-0 md:hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-xl mt-2 flex flex-col gap-2">
          ...
          <a
            ...
            rel="noopener noreferrer"
            onClick={() => setIsOpen(false)}
            className="mt-2 text-center bg-indigo-600 hover:bg-indigo-500 text-white text-xs px-4 py-3 rounded-xl font-bold transition-colors"
          >
            DOWNLOAD CV
          </a>
        </div>
